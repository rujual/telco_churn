...
cdc = centroid_df.drop(['tenure','MonthlyCharges','TotalCharges'], axis=1).applymap(roundoff)
cdn = centroid_df[['tenure','MonthlyCharges','TotalCharges']]
cdf = cdn.join(cdc)
cdf = cdf[centroid_df.columns]
...
for x in decode_cols:
	cdf[x] = pd.Series(['tab' for x in range(len(cdf.index))])
...
for x in dic_decode.keys():
	for y in list(dic_decode[x]):
        cdf[cdf[y==1]][x]=str(y).split("#")[1]
        cdf.drop(str(y), axis=1, inplace=True)

cdf['PaymentMethod'].replace({"Yes":1,"No":0}, inplace=True)
...
if st.sidebar.button('Search'):
    ...
    my_dict = {'customerID':df_sear['customerID'].iloc[0],'gender':df_sear['gender'].iloc[0],'SeniorCitizen':df_sear['SeniorCitizen'].iloc[0],
...
for x in df_cat1:
    ...
    temp = st.sidebar.selectbox(label=s, index=l_ind, options=l_unique, key=str(x)) # index=(l_unique.index(df_sear[x])),
    select_param[x]=temp
...
for y in df_num1:
    ...
    temp1 = st.sidebar.slider(label=s, min_value=(dfst[y].unique()).astype(float).min(),
                              ...
                              step=(((dfst[y].unique()).astype(float).max())-int((dfst[y].unique()).astype(float).min()))/10,
                              key=str(y))
